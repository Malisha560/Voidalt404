...
function Verification() {
  ...
  useEffect(() => {
    fetchInvigilatorStudent(id).then((data) => { setStudent(data); setSelectedExamId(String(data.exams?.[0]?.id || '')) }).catch(() => setError('Unable to load student information.')).finally(() => setLoading(false))
  }, [id])
  ...
  const updateAttendance = async (status) => {
    ...
    setError('')
    try {
      await updateInvigilatorAttendance(student.id, selectedExamId, status)
      setActionState(`${status}_DONE`)
    } catch (requestError) {
      setActionState('')
  ...
  return <main className="student-card-page">
    ...
    <section className={`student-id-card ${student.feeStatus === 'CLEAR' ? 'fee-clear-card' : 'fee-unclear-card'}`}>
      ...
      <div className="id-card-body"><div className="id-card-accent"><span>{student.course}</span></div><div className="student-photo">{student.profileImage ? <img src={student.profileImage} alt={`${student.name} profile`} /> : <span>{initials}</span>}</div><dl className="student-card-details"><div><dt>Name:</dt><dd>{student.name}</dd></div><div><dt>ID:</dt><dd>{student.email}</dd></div><div><dt>Contact:</dt><dd>{student.phoneNumber}</dd></div><div><dt>Student ID:</dt><dd>{student.studentId}</dd></div><div><dt>Level:</dt><dd>{student.year ? `Level ${student.year}` : 'Not specified'}</dd></div><div><dt>Fee Status:</dt><dd className={student.feeStatus === 'CLEAR' ? 'clear' : 'unclear'}>{student.feeStatus}</dd></div>{student.feeStatus === 'CLEAR' && <div><dt>Exam:</dt><dd><select value={selectedExamId} onChange={(event) => setSelectedExamId(event.target.value)} aria-label="Select examination">{student.exams?.map((exam) => <option value={exam.id} key={exam.id}>{exam.subject}</option>)}</select></dd></div>}</dl>{student.feeStatus === 'CLEAR' && <div className="id-card-actions"><button className="approve-card-button" type="button" disabled={Boolean(actionState)} onClick={() => updateAttendance('PRESENT')} aria-label="Approve student">{actionState === 'PRESENT' || actionState === 'PRESENT_DONE' ? '✓' : '✓'}</button><button className="reject-card-button" type="button" disabled={Boolean(actionState)} onClick={() => updateAttendance('ABSENT')} aria-label="Reject student">×</button>{actionState === 'PRESENT_DONE' && <small className="attendance-confirmation">Marked present</small>}{actionState === 'ABSENT_DONE' && <small className="attendance-confirmation rejected">Marked absent</small>}</div>}</div><footer>ISLINGTON COLLEGE KATHMANDU</footer>
    </section>
  </main>
}
